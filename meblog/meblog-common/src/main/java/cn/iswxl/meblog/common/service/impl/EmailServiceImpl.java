...
public class EmailServiceImpl implements EmailService {
    ...
    @Override
    public void send(SendEmailRspVO sendEmailRspVO) {

        // 读取邮箱配置
        if (email == null || host == null || port == null || username == null || password == null) {
            throw new RuntimeException("邮箱配置信息异常");
        }
        ...
        // 发送邮件
        try {
            int size = sendEmailRspVO.getTos().size();
            Mail.create(account)
                    .setTos(sendEmailRspVO.getTos().toArray(new String[size]))
                    .setTitle(sendEmailRspVO.getSubject())
                    .setContent(sendEmailRspVO.getContent())
                    .setHtml(true)
                    // 关闭session
